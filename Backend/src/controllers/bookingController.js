...
const createBooking = async (req, res) => {
  try {
    ...
    if (transportationId) {
      ...
      // Preserve distance-based calculation from HEAD
      if (tourPackId) {
        const tp = await TourPack.findById(tourPackId);
        if (tp && tp.distance) {
          transportationAmount = transportation.price * tp.distance;
        }
      }
    }
...
const getMyBookings = async (req, res) => {
  try {
    ...
    const bookingFilter = normalizedEmail
      ? {
          $or: [
            { user: req.user._id },
    ...
    const legacyBookingIds = bookings
      .filter(
        (booking) =>
          !booking.user && String(booking.email || '').trim().toLowerCase() === normalizedEmail
      )
      .map((booking) => booking._id);

    if (legacyBookingIds.length > 0) {
      await Booking.updateMany(
        { _id: { $in: legacyBookingIds } },
